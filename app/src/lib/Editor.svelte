<script>
  ...
  let innerHtml = $state('')
  let title = $state('')
  let fontFamily = $state('AGaramond')
  let editTitleToggle = $state(false)

  // Initialize from localStorage on mount
  onMount(() => {
    if (typeof window !== 'undefined') {
      innerHtml = localStorage.getItem(`${editorName}Html`) || ''
      title = localStorage.getItem(`${editorName}Title`) || ''
      fontFamily = localStorage.getItem(`${editorName}Font`) || 'AGaramond'
    }
  })
  ...
  function updateWordCount(innerText) {
    ...
    wordCount.update(wc => ({ ...wc, [editorName]: count }))
  }
...
<div>
  <!-- Header -->
  <div class="header" class:dark-mode={$dark}>
    ...
    <div class="header-right">
      <SvelteTooltip tip="Copy & Save" bottom color={$dark ? '#71318d' : '#f7e5ff'}>
        <button class="duck" class:dark-mode={$dark} onclick={save}> 💾 & 📋 </button>
      </SvelteTooltip>
